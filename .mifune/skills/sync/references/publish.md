...
```bash
# NOTE: this branch is rooted at upstream/development, which has the NON-relocated
# layout — skills live at .claude/skills/, not .mifune/skills/. All path references
# here use upstream's layout, not origin's.
#
...
# Research wiki corpus — keep README anchors only (upstream layout: .claude/skills/)
...
`## [X.Y.Z]` sections (they are already there). Public convention keeps
`ryaneggz` issue links in new entries.
...
```bash
git diff upstream/development HEAD | grep '^+' | grep 'ryaneggz/openharness'
```
...
```bash
git diff upstream/development HEAD -- ':!CHANGELOG.md' | grep '^+' | grep 'ryaneggz/openharness'
```

Don't over-sanitize: pre-existing `ryaneggz` refs in OLDER released
CHANGELOG sections or test fixtures from before the sync are NOT a leak —
scope the sed/replacement to files the sync INTRODUCED.
...
Note: on the upstream branch (non-relocated `.claude/skills` layout), the
runner path is `bash .claude/skills/eval/run.sh`. Match the branch layout.
