...
- **`.oh/`** — *OpenHarness's own machinery* as one unit: the `oh` CLI (`cli/`),
  ...
  (`install/`), the always-on identity core (`context/`), and user-local deploy
  config (`config.json`). The former
...
- **repo root** — everything forced to root by *external* tooling
  ...
  identity/state the harness edits in place (`evals/`, `crons/`,
...
|------|------|
| OpenHarness's own machinery addressed as a unit: the `oh` CLI, the docs-site builder, installer/lifecycle scripts, container-install inputs, deploy/compose config, the always-on identity core (`context/`) | Surfaces **forced to root by external tooling** (`.devcontainer/`, `harness.yaml`, `package.json`, `pnpm-*.yaml`, `.github/`, `.husky/`) and **live identity/state** edited in place (`evals/`, `crons/`, `memory/`, `tasks/`, `workspace/`, and the `docs/`+`blog/` markdown content) |
