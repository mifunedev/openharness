...
scope-boundary line (e.g. `.oh/skills/<skill>/SKILL.md`) and `Grep`/`Bash` the relevant
...
`.claude/agents/` is a symlink to `.oh/agents/`, so writing this file to
`.oh/agents/auditor.md` **auto-registers `auditor` as a `subagent_type`** — no registry
