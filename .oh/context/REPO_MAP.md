...
6. Prefer curated `.oh/skills/wiki/corpus/*.md` and `.oh/context/IDENTITY.md` over raw logs/snapshots.
...
| Sandbox lifecycle, Docker, provisioning | `Makefile`, `.devcontainer/` (incl. `.example.env` and local `.env`), `.oh/scripts/README.md`, `.oh/scripts/docker-compose.sh` | Owns container image, compose overlays, env, and lifecycle commands. |
...
| `.devcontainer/.example.env` / local `.devcontainer/.env` | The tracked configuration schema plus the operator-local values generated from it. Since 0.4.0 this is the ONE config surface, read on every path including VS Code. | Inspect supported keys in the template; inspect the local `.env` only when debugging the current checkout. |
