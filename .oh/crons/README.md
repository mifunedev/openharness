...
worktree: false          # optional — run each fire in a fresh .oh/worktrees/cron/<session> (root stays clean; never SKIPPED_OVERLAP)
...
| `SPAWNED_WORKTREE` | A `worktree: true` fire launched its detached session inside a fresh isolated `.oh/worktrees/cron/<session>` worktree (`msg` is `<session> <worktree-path>`); the shared root checkout is untouched. |
