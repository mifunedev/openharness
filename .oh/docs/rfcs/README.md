...
| [#806](https://github.com/mifunedev/openharness/issues/806) | Draft | #592 child. The [companion spec](rfc-gvisor-support.md) proposes gVisor (`runsc`) as the first supported A1 substrate beyond the default container, on a measured basis: boots, holds a detached tmux session, runs nested `dockerd`, costs 1.15x wall and 2.05x CPU. Records the MicroSandbox glibc 2.39 blocker and the Docker Desktop socket prerequisite. Operator procedures live in the [host runbook](../gvisor-host-runbook.md). Decision artifact — no runtime change. |
