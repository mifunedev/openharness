...
| [Bounded local CodeLayer coding-harness RFC](rfc-codelayer-remote-daemon.md) ([#635](https://github.com/mifunedev/openharness/issues/635)) | Accepted (local coding harness only); remote-daemon appendix Draft | Exact `@humanlayer/codelayer@0.0.61` local Bun-source wrapper and explicit Ralph adapter are accepted from no-egress image evidence; `@humanlayer/cli`, daemon/login/token/browser/control-plane behavior remains deferred and unaccepted. |
