...
| [gVisor A1 substrate](rfc-gvisor-support.md) | Draft | #592 child. Proposes gVisor (`runsc`) as the first supported A1 substrate beyond the default container, on a measured basis: boots, holds a detached tmux session, runs nested `dockerd`, costs 1.15x wall and 2.05x CPU. Records the MicroSandbox glibc 2.39 blocker and the Docker Desktop socket prerequisite. Measurement [#802](https://github.com/mifunedev/openharness/issues/802) / [#803](https://github.com/mifunedev/openharness/pull/803). Decision artifact — no runtime change. |
