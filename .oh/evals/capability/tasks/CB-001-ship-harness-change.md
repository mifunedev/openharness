...
## Scoring method
v1: inspect the most-recent real instance of this capability — the latest autopilot-shipped or `/ship-spec`-shipped PR — against the rubric. Confirm the branch name shape, `Closes #N`, the `tasks/<slug>/` scaffold (`prd.md`+`prd.json`), CI status via `gh pr checks`, and ready (non-draft) state via `gh pr view --json isDraft`. For the eval axis, diff the PR's `.oh/evals/RESULTS.md` against its base to confirm no green→red row. Alternatively, run a fresh request through `/ship-spec` and score the produced PR.
