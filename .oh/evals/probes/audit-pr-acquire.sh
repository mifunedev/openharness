...
for n in 1 2; do PATH="$bin:$PATH" TMPDIR="$tmp" AUDIT_RUN_ID="audit-20260717T12000${n}Z-c$n" bash "$A" pr --repo o/r --pr 7 >"$tmp/$n.out" & pids[n]=$!; done
wait "${pids[1]}"; wait "${pids[2]}"
for n in 1 2; do jq -e '.mode=="pr" and .prs[0].number==7 and .prs[0].statusCheckRollup[0].status=="COMPLETED"' "$tmp/$n.out" >/dev/null || fail "focused envelope $n"; done
