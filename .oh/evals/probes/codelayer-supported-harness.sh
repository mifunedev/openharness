...
has 'tmux new-session -E "${TMUX_ENV_ARGS[@]}"' "$RALPH" "per-session tmux environment array"
has 'OPENAI_API_KEY ANTHROPIC_API_KEY FIREWORKS_API_KEY EXA_API_KEY' "$RALPH" "pinned-source credential allowlist"
has 'AGENTLAYER_AUTH_PATH AGENT_SDK_AUTH_PATH OPENCODE_AUTH_PATH' "$RALPH" "documented AgentLayer auth path allowlist"
has 'TMUX_UNSET_CMD+="unset $env_name; "' "$RALPH" "stale allowlisted server values removed when caller leaves them unset"
