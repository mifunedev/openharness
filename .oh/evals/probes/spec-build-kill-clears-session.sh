...
#         `spec-build.sh --kill <slug>` exited 1 with NO output, left the
#         agent-build-<slug> tmux session running, left /tmp/build-<slug>.lock
#         claimed, and appended no BUILD-SESSION-INCOMPLETE line — because teardown aborted
...
for f in "$SPEC_BUILD" "$RUNNER" "$CONTRACT"; do
  if [ ! -f "$f" ]; then
...
if command -v tmux >/dev/null 2>&1 && tmux -V >/dev/null 2>&1; then
  if tmux new-session -d -s "agent-build-$DECOY" 'sleep 600' 2>/dev/null; then
    tmux_used=1
  fi
fi
mkdir -p "$TMPD/build-$DECOY.lock"

kill_out="$(cd "$WORK/repo" && RUNNER_TMPDIR="$TMPD" bash .oh/scripts/spec-build.sh --kill "$DECOY" 2>&1)"
kill_rc=$?
...
[ ! -e "$TMPD/build-$DECOY.lock" ] \
  || missing+=("--kill left the launch lock claimed at \$RUNNER_TMPDIR/build-$DECOY.lock")
grep -q '^BUILD-SESSION-INCOMPLETE ' "$WORK/repo/.oh/tasks/$DECOY/progress.txt" \
  || missing+=("--kill appended no BUILD-SESSION-INCOMPLETE line to the decoy progress.txt")

if [ "$tmux_used" -eq 1 ]; then
  if tmux has-session -t "agent-build-$DECOY" 2>/dev/null; then
    missing+=("--kill left the tmux session agent-build-$DECOY running")
    tmux kill-session -t "agent-build-$DECOY" 2>/dev/null || true
  fi
else
...
if [ "${#missing[@]}" -gt 0 ]; then
  printf 'REGRESSION: spec-build.sh --kill does not clear the session:\n' >&2
  printf '  - %s\n' "${missing[@]}" >&2
  exit 1
fi

echo "PASS: --kill exits 0 with output, clears the lock, appends BUILD-SESSION-INCOMPLETE, tears the session down, and a non-zero herdr pane lookup no longer aborts teardown" >&2
exit 0
