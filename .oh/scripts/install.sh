...
# Gitignored host-local settings generated by install.sh. Keeping host
# defaults here (not in shared config) lets future installer runs pull cleanly.
#
...
# Materialize harness.yaml from the tracked template when missing, so the
# one-line (curl) install converges on the same config state as `make sandbox`
# (which runs the harness-config target first). Peer impl: Makefile:37-43.
# harness.yaml is gitignored; the .example is tracked and ships fully commented,
# so this copy is inert — it never overrides the .devcontainer/.env defaults
# written above; it just gives the user the file the quickstart tells them to edit.
if [ ! -f "$REPO_DIR/harness.yaml" ]; then
  if [ -f "$REPO_DIR/harness.yaml.example" ]; then
    ...
    ok "Created harness.yaml from harness.yaml.example (edit to customize; harness.yaml overrides local env)"
  else
...
printf "  ──────────────────────────────────────\n"
printf "       ${CYAN}harness.yaml${NC}        — local (gitignored) shared config (active keys override local env).\n"
...
printf "\n"
printf "  ${CYAN}Optional capabilities${NC}  (enable in harness.yaml, then: make destroy && make sandbox)\n"
printf "  ──────────────────────────────────────\n"
