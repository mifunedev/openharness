...
Validate a positive PR number and `--repo owner/name` before starting. Acquire with `$AUDIT_ROOT/.oh/skills/audit/scripts/pr-acquire.sh pr --repo "$REPO" --pr "$N"`, then pipe the schema-versioned envelope to `$AUDIT_ROOT/.oh/skills/audit/scripts/pr-classify.sh`. Acquisition failure or incomplete evidence is unknown; never infer readiness.
