#!/usr/bin/env bash
# .oh/skills/autopilot/autopilot-caps.sh — deterministic autopilot PR-cap
...
# This script lives in <root>/.oh/skills/autopilot/ but reads root-level
...
# agent-symlinks (.claude/skills) or the neutral .oh/skills source, so walk
# upward to find the repo root (the dir holding .oh/scripts/cron-runtime.ts) instead
# of hard-coding a fixed depth — the same idiom as .oh/skills/eval/run.sh.
