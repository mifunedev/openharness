...
Per `.oh/skills/retro/references/memory-protocol.md`, append to `.oh/memory/<UTC-date>/log.md` (create the dated directory if missing):
...
Then run the qualify/improve loop per `.oh/skills/retro/references/memory-protocol.md`. Lessons about scenario-quality patterns (e.g. "scenarios under 10 words usually need a follow-up `/prd` pass") may belong in `.oh/memory/MEMORY.md`.
