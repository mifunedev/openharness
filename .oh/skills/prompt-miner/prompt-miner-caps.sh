#!/usr/bin/env bash
# .oh/skills/prompt-miner/prompt-miner-caps.sh — origin-scoped PR-cap
...
# (.oh/skills/autopilot/autopilot-caps.sh) defaults to mifunedev/openharness
...
# The canonical gate is the sibling skill's script under .oh/skills/autopilot/
# (both live one level below .oh/skills/), so resolve it relative to here.
