...
description: |
  ...
  .oh/scripts/firstmate.sh (the recursion loop) and .oh/worktrees/ (isolated branches) BY
...
|---|---|---|
| Recursion **loop** | `.oh/scripts/firstmate.sh` | each story re-reads disk = the REPL step — **reused by reference, never edited** |
...
> Do **not** edit `.oh/scripts/firstmate.sh` or anything under `.oh/worktrees/`. `/rlm` is a
...
**reuses `.oh/scripts/firstmate.sh`** (each story re-reads disk = the REPL step) and
...
- **Editing the reused substrate.** `.oh/scripts/firstmate.sh` and `.oh/worktrees/` are reused
  by reference. Editing either is out of scope (and firstmate.sh is explicitly off-limits
