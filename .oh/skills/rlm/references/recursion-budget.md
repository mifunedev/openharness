...
[`.oh/skills/advisor/references/recursive-delegation.md`](../../advisor/references/recursive-delegation.md)
...
| **Callee** | `/weigh` (`.oh/skills/weigh/SKILL.md`) |
...
| **Cohort shape** | Each candidate is a trajectory record matching the **`TRAJECTORY_SCHEMA`** exported by `.oh/skills/weigh/scripts/score-trajectories.mjs` (`id`, `output`, `costTokens`, `evalRc`, `auditVerdict`, `clusterId`, `clusterSize`, `judgeScore`, `judgeReason`) |
...
- `.oh/skills/advisor/references/recursive-delegation.md` — the budget triple + multi-level delegation protocol this reference points at.
