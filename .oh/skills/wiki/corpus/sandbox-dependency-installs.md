...
## Detail
The root install block explains the runtime shadowing problem and uses `SKIP_PNPM_INSTALL=1` as the explicit opt-out for externally managed or air-gapped dependency state (`.devcontainer/entrypoint.sh:312`, `.devcontainer/entrypoint.sh:320`). It derives workspace package patterns from `pnpm-workspace.yaml`, ignores negated patterns, and does not include undeclared package-local manifests (`.devcontainer/entrypoint.sh:322`, `.devcontainer/entrypoint.sh:338`). Manifest discovery excludes dependency and runtime/vendor paths such as `.git`, `.worktrees`, `node_modules`, `.pi/npm/node_modules`, `.oh/cli/node_modules`, and `.hermes/lsp/node_modules` (`.devcontainer/entrypoint.sh:367`).
...
The Vitest file asserts the marker filename and location, the `pnpm_manifest_fingerprint` helper shape, drift reinstall branch, current-dependencies skip branch, atomic refresh, and install-failure abort behavior (`.oh/scripts/__tests__/entrypoint-pnpm-install.test.ts:19`, `.oh/scripts/__tests__/entrypoint-pnpm-install.test.ts:24`, `.oh/scripts/__tests__/entrypoint-pnpm-install.test.ts:32`, `.oh/scripts/__tests__/entrypoint-pnpm-install.test.ts:38`, `.oh/scripts/__tests__/entrypoint-pnpm-install.test.ts:43`, `.oh/scripts/__tests__/entrypoint-pnpm-install.test.ts:50`). The Tier-A probe checks the same contract from the eval suite and returns a regression if any expected source or test signal disappears (`.oh/evals/probes/entrypoint-pnpm-manifest-fingerprint.sh:30`, `.oh/evals/probes/entrypoint-pnpm-manifest-fingerprint.sh:40`, `.oh/evals/probes/entrypoint-pnpm-manifest-fingerprint.sh:47`, `.oh/evals/probes/entrypoint-pnpm-manifest-fingerprint.sh:58`).
