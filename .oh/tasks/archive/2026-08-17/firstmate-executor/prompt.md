# Ralph iteration — firstmate-executor

You are one iteration of a Ralph loop implementing the `firstmate-executor` task. The full plan is in `.oh/tasks/firstmate-executor/prd.md` and the structured task list is in `.oh/tasks/firstmate-executor/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/firstmate-executor/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/firstmate-executor/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/firstmate-executor/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/firstmate-executor/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `feat/746-firstmate-executor` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.oh/worktrees/feat/746-firstmate-executor`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b feat/746-firstmate-executor "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout feat/746-firstmate-executor
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/746-firstmate-executor`.
...
- PRD: `.oh/tasks/firstmate-executor/prd.md`
- Structured stories: `.oh/tasks/firstmate-executor/prd.json`
- Branch: `feat/746-firstmate-executor`
- Issue: #746
