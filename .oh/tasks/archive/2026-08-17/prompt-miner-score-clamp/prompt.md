# Ralph iteration — prompt-miner-score-clamp

You are one iteration of a Ralph loop implementing the `prompt-miner-score-clamp` task. The full plan is in `.oh/tasks/prompt-miner-score-clamp/prd.md` and the structured task list is in `.oh/tasks/prompt-miner-score-clamp/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/prompt-miner-score-clamp/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/prompt-miner-score-clamp/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/prompt-miner-score-clamp/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/prompt-miner-score-clamp/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `fix/778-prompt-miner-score-clamp` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.oh/worktrees/fix/778-prompt-miner-score-clamp`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b fix/778-prompt-miner-score-clamp "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout fix/778-prompt-miner-score-clamp
   ```
...
- **Never push** to `development` or `main`. Branch is `fix/778-prompt-miner-score-clamp`.
...
- PRD: `.oh/tasks/prompt-miner-score-clamp/prd.md`
- Structured stories: `.oh/tasks/prompt-miner-score-clamp/prd.json`
- Branch: `fix/778-prompt-miner-score-clamp`
- Issue: #778
