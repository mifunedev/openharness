# Ralph iteration — compose-dry-run-env-files

You are one iteration of a Ralph loop implementing the `compose-dry-run-env-files` task. The full plan is in `.oh/tasks/compose-dry-run-env-files/prd.md` and the structured task list is in `.oh/tasks/compose-dry-run-env-files/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/compose-dry-run-env-files/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/compose-dry-run-env-files/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/compose-dry-run-env-files/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/compose-dry-run-env-files/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-compose-dry-run-env-files` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-compose-dry-run-env-files`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b <prefix>/<N>-compose-dry-run-env-files "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout <prefix>/<N>-compose-dry-run-env-files
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-compose-dry-run-env-files`.
...
- PRD: `.oh/tasks/compose-dry-run-env-files/prd.md`
- Structured stories: `.oh/tasks/compose-dry-run-env-files/prd.json`
- Branch: `<prefix>/<N>-compose-dry-run-env-files`
- Issue: #470
