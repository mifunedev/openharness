...
# First Mate session — debian-trixie

You are the **First Mate** running one long-lived session that implements the
`debian-trixie` task end to end. Unlike a ralph loop — 50 fresh processes, one story
each — you hold the **whole task graph** in a single session and walk it
story by story.

- Task folder: `.oh/tasks/debian-trixie/` (`prd.md`, `prd.json`, `prompt.md`, `progress.txt`)
- Branch: `task/807-debian-trixie` — never push to `development` or `main`
- Issue: #807

## 1. Load the task graph

Read `.oh/tasks/debian-trixie/prd.md` for intent and the `## Codebase Patterns`
...
1. `STATUS: COMPLETE` as a **whole line** in `.oh/tasks/debian-trixie/progress.txt`.
...
`.oh/tasks/debian-trixie/evidence.md` per
...
- PRD: `.oh/tasks/debian-trixie/prd.md`
- Structured stories: `.oh/tasks/debian-trixie/prd.json`
- Progress + terminal marker: `.oh/tasks/debian-trixie/progress.txt`
- Branch: `task/807-debian-trixie`
- Issue: #807
