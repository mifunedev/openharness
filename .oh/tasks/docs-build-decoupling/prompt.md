...
You are one iteration of a Ralph loop implementing the `docs-build-decoupling` task. The full plan is in `.oh/tasks/docs-build-decoupling/prd.md` and the structured task list is in `.oh/tasks/docs-build-decoupling/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/docs-build-decoupling/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/docs-build-decoupling/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/docs-build-decoupling/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/docs-build-decoupling/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
- PRD: `.oh/tasks/docs-build-decoupling/prd.md`
- Structured stories: `.oh/tasks/docs-build-decoupling/prd.json`
