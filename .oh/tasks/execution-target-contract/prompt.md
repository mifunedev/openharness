# Ralph iteration — execution-target-contract

You are one iteration of a Ralph loop implementing the `execution-target-contract` task. The full plan is in `.oh/tasks/execution-target-contract/prd.md` and the structured task list is in `.oh/tasks/execution-target-contract/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/execution-target-contract/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/execution-target-contract/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/execution-target-contract/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/execution-target-contract/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `feat/733-execution-target-contract` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.oh/worktrees/feat/733-execution-target-contract`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b feat/733-execution-target-contract "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout feat/733-execution-target-contract
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/733-execution-target-contract`.
...
- PRD: `.oh/tasks/execution-target-contract/prd.md`
- Structured stories: `.oh/tasks/execution-target-contract/prd.json`
- Branch: `feat/733-execution-target-contract`
- Issue: #733
