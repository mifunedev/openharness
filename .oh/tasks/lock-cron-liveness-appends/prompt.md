# Ralph iteration — lock-cron-liveness-appends

You are one iteration of a Ralph loop implementing the `lock-cron-liveness-appends` task. The full plan is in `.oh/tasks/lock-cron-liveness-appends/prd.md` and the structured task list is in `.oh/tasks/lock-cron-liveness-appends/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/lock-cron-liveness-appends/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/lock-cron-liveness-appends/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/lock-cron-liveness-appends/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/lock-cron-liveness-appends/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-lock-cron-liveness-appends` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-lock-cron-liveness-appends`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b <prefix>/<N>-lock-cron-liveness-appends "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout <prefix>/<N>-lock-cron-liveness-appends
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-lock-cron-liveness-appends`.
...
- PRD: `.oh/tasks/lock-cron-liveness-appends/prd.md`
- Structured stories: `.oh/tasks/lock-cron-liveness-appends/prd.json`
- Branch: `<prefix>/<N>-lock-cron-liveness-appends`
- Issue: #474
