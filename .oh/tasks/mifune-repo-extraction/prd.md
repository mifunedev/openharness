...
- [ ] Record the current `ryaneggz/mifune` default branch name and HEAD SHA in `.oh/tasks/mifune-repo-extraction/progress.txt` before replacing contents, so rollback can restore the previous default if needed.
...
- [ ] Commit and push the checkpoint/replacement; record the resulting final `ryaneggz/mifune` default-branch SHA in `.oh/tasks/mifune-repo-extraction/progress.txt` and the Open Harness PR body.
...
- [ ] Preserve executable bits for all executable files under `.mifune/hooks/` and any executable Mifune scripts/cap files; compare before/after mode lists in `.oh/tasks/mifune-repo-extraction/progress.txt`.
