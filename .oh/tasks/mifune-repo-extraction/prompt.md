# Ralph iteration — mifune-repo-extraction

You are one iteration of a Ralph loop implementing the `mifune-repo-extraction` task. The full plan is in `.oh/tasks/mifune-repo-extraction/prd.md` and the structured task list is in `.oh/tasks/mifune-repo-extraction/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/mifune-repo-extraction/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/mifune-repo-extraction/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/mifune-repo-extraction/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/mifune-repo-extraction/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `task/539-mifune-repo-extraction` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/task/539-mifune-repo-extraction`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b task/539-mifune-repo-extraction "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout task/539-mifune-repo-extraction
   ```
...
- **Never push** to `development` or `main`. Branch is `task/539-mifune-repo-extraction`.
...
- PRD: `.oh/tasks/mifune-repo-extraction/prd.md`
- Structured stories: `.oh/tasks/mifune-repo-extraction/prd.json`
- Branch: `task/539-mifune-repo-extraction`
- Issue: #539
