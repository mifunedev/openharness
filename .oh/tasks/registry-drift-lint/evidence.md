...
All four checks pass on PR #765 at `dafa8232`; the PR is `MERGEABLE` / `CLEAN`
and not a draft.

```text
$ gh pr checks 765 --repo mifunedev/openharness
Boot Path Lint (shellcheck + hadolint)      pass   15s
Eval Probe Regression Gate                  pass   29s
Lint, Typecheck, Build & Test               pass   47s
Validate sandbox compose and image build    pass   2m14s

$ gh pr view 765 --json mergeable,mergeStateStatus
mergeable=MERGEABLE state=CLEAN
```

Two of these bear directly on this change. `Boot Path Lint` is the gate whose
shellcheck argument list covers `.oh/scripts/*.sh`, which is why the linter was
placed there rather than under `.oh/docs/` or a `/sync` subcommand. `Eval Probe
Regression Gate` runs the probe corpus in CI, so `registry-portability-gate.sh`
is executed on every future run — unlike `registry-portability.sh`, which finds
no checkout there and reports SKIPPED.

The branch was merged with `development` twice during this work: once at
`ec64f667` (clean) and once at `e4af96d4`, where `.oh/evals/RESULTS.md`
conflicted and was **regenerated by re-running `/eval`** rather than
hand-resolved, per the documented procedure. `CHANGELOG.md` auto-merged with
both entries intact.
