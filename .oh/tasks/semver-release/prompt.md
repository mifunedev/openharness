# Ralph iteration — semver-release

You are one iteration of a Ralph loop implementing the `semver-release` task. The full plan is in `.oh/tasks/semver-release/prd.md` and the structured task list is in `.oh/tasks/semver-release/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/semver-release/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/semver-release/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/semver-release/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/semver-release/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `task/814-semver-release` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.oh/worktrees/task/814-semver-release`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b task/814-semver-release "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout task/814-semver-release
   ```
...
- **Never push** to `development` or `main`. Branch is `task/814-semver-release`.
...
- PRD: `.oh/tasks/semver-release/prd.md`
- Structured stories: `.oh/tasks/semver-release/prd.json`
- Branch: `task/814-semver-release`
- Issue: #814
