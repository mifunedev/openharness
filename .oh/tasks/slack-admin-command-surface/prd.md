...
Issue #354 reports that the Slack bridge docs imply `/help`, `/trusted`, `/channels`, `/enable`, and `/disable` are visible agent/Pi commands, but the current implementation exposes only `/msg-bridge` as a Pi command. The admin commands are bridge-owned Slack DM text handlers and may be intercepted by Slack's slash-command UI before reaching the bot.
...
- Do not implement Slack-native slash commands without adding Slack manifest command definitions and package-owned Bolt command handlers.
...
- `.pi/install/slack-manifest.json` contains message event subscriptions and no Slack-native slash command definitions.
...
- Docs state that bridge admin commands are Slack DM text handlers after trust is established.
...
- Docs explain that these are not Slack-native slash commands in the shipped manifest.
