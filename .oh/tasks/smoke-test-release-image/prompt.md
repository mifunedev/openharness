# Ralph iteration — smoke-test-release-image

You are one iteration of a Ralph loop implementing the `smoke-test-release-image` task. The full plan is in `.oh/tasks/smoke-test-release-image/prd.md` and the structured task list is in `.oh/tasks/smoke-test-release-image/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/smoke-test-release-image/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/smoke-test-release-image/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/smoke-test-release-image/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/smoke-test-release-image/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-smoke-test-release-image` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-smoke-test-release-image`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b <prefix>/<N>-smoke-test-release-image "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout <prefix>/<N>-smoke-test-release-image
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-smoke-test-release-image`.
...
- PRD: `.oh/tasks/smoke-test-release-image/prd.md`
- Structured stories: `.oh/tasks/smoke-test-release-image/prd.json`
- Branch: `<prefix>/<N>-smoke-test-release-image`
- Issue: #515
