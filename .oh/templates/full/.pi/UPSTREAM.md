...
- The pin in `.oh/scripts/gateway.sh`'s `FORK_PIN` **is** the review/bump artifact. It points at exact tested commit `git+https://github.com/ryaneggz/pi-messenger-bridge.git#a445513961af60b11f00d0ef9f55a58e5e14fabd` from [ryaneggz/pi-messenger-bridge#2](https://github.com/ryaneggz/pi-messenger-bridge/pull/2); once upstream publishes a release containing these changes, re-pin to `pi-messenger-bridge@<release>`.
