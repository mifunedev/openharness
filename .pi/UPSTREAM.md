...
- The pin in `.oh/scripts/gateway.sh`'s `FORK_PIN` **is** the review/bump artifact. It currently points at the fork branch `github:ryaneggz/pi-messenger-bridge#feat/slack-thread-replies-admin-commands` (Slack thread-reply patch plus admin slash-command handlers; fork PR [ryaneggz/pi-messenger-bridge#1](https://github.com/ryaneggz/pi-messenger-bridge/pull/1)); once upstream merges and publishes a release, re-pin it to `pi-messenger-bridge@<release>`.
