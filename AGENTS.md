...
| `/autopilot-watchdog` | Ad-hoc + heartbeat watchdog for `/autopilot` — kills only truly frozen tmux sessions, surfaces long-lived sessions, ready non-draft PRs, draft PRs stale after 2h, and draft-cap saturation; PR handling is read-only |
