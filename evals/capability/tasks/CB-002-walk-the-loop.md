...
## Success signal
- `/loop-runner --dry-run --start ideate` exits `0` and prints a **connected** route — every node's declared terminal STATUS token resolves to a real next node via the `context/rules/loop.md` § 2 route table.
