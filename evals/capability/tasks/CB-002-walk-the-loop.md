...
## Success signal
- `/loop --dry-run --start ideate` exits `0` and prints a **connected** route — every node's declared terminal STATUS token resolves to a real next node via the `context/rules/loop.md` § 2 route table.
...
## Evidence basis
The `/loop` runner plus `context/rules/loop.md` § 2 route table and the `loop-handoff-consistency` probe (executable-loop series) demonstrate this capability. Note: the loop machinery lives in the executable-loop series (#157/#163), currently being integrated into `development`; this task scores the harness's loop-walking capability there, not on a branch where the runner is absent. The runner is mechanical by design — it routes, it does not decide (loop.md § 6) — so an honest halt at an unwired node is the correct PASS, not a failure.

## Scoring method
v1: against the branch carrying the executable-loop series, run `/loop --dry-run --start ideate` fresh and inspect its output against the rubric — confirm exit 0, a connected route, the reported honest-halt node, and that each printed edge maps to a § 2 route-table entry. Then run the `loop-handoff-consistency` probe and confirm green. If the series is not yet present on the branch under evaluation, mark this task SKIPPED (capability not present here) rather than FAIL.
