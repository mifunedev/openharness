...
## Success signal
- The pipeline (driven by `/autopilot` → `/ship-spec`, or the decomposed `/spec plan` → `/spec critique` → `/spec execute`) produces a `.oh/tasks/<slug>/` four-file folder before any build.
