...
grep -Fq '>> "$AUTOPILOT_LOG_ROOT/crons/.cron.log"' "$SKILL" || missing+=("liveness appends through AUTOPILOT_LOG_ROOT")
grep -Fq 'cat >> "$AUTOPILOT_LOG_ROOT/memory/$TODAY/log.md"' "$SKILL" || missing+=("memory log appends through AUTOPILOT_LOG_ROOT")
