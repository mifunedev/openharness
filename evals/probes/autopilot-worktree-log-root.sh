...
grep -Fq 'scripts/locked-append.sh "$AUTOPILOT_LOG_ROOT/crons/.cron.log"' "$SKILL" || missing+=("liveness appends through AUTOPILOT_LOG_ROOT via locked append")
grep -Fq 'scripts/locked-append.sh "$AUTOPILOT_LOG_ROOT/memory/$TODAY/log.md"' "$SKILL" || missing+=("memory log appends through AUTOPILOT_LOG_ROOT via locked append")
