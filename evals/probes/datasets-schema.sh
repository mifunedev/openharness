#!/usr/bin/env bash
# tier: A
# source: issue #196 — evals/datasets verifiable trajectory corpus (Repo2RLEnv-inspired)
...
if (( ${#example_dirs[@]} < 3 )); then
  fails+=("expected >=3 example folders <dataset>/DS-*/ under evals/datasets, found ${#example_dirs[@]}")
fi
...
CAP_TASKS="$ROOT/evals/capability/tasks"
