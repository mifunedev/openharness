...
if [ -f "$REPO_DIR/.devcontainer/.env" ]; then
  ...
  __TZ="$(cat /etc/timezone 2>/dev/null || echo Etc/UTC)"
  ...
  # ── Secrets-only .env (consistent with .devcontainer/.example.env) ────
  cat > "$REPO_DIR/.devcontainer/.env" <<'ENVEOF'
# ─── Open Harness — Secrets Configuration ───────────────────────────
# SECRETS ONLY. Everything else (sandbox name, timezone, git identity,
# install flags, cron settings, Slack config, compose overlays) lives in
# tracked harness.yaml at the repo root. Keys set in harness.yaml WIN
# over this file.
#
# Generated by install.sh on first install. Edit freely; rerun
...
ENVEOF
  ok "Wrote .devcontainer/.env (secrets only)"

  # ── Update harness.yaml with host-detected non-secret values ──────────
  # Guards against absent harness.yaml (graceful degradation).
  if [ -f "$REPO_DIR/harness.yaml" ]; then
    __YAML="$REPO_DIR/harness.yaml"

    # sandbox.name: uncomment if commented, then replace the value field
    # (idempotent: both patterns run; [^[:space:]]* stops before any
    # trailing inline comment on an already-active line).
    __SN_SAFE="$(_sed_val "$SANDBOX_NAME")"
    _sedi "s|^  # name:.*$|  name: ${__SN_SAFE}|" "$__YAML"
    _sedi "s|^\(  name: \)[^[:space:]]*|\1${__SN_SAFE}|" "$__YAML"
    ok "harness.yaml: sandbox.name → $SANDBOX_NAME"
    unset __SN_SAFE

    # sandbox.timezone: uncomment if commented, then set value (idempotent:
    # both patterns run; second is a no-op if first already matched).
    if [ -n "$__TZ" ]; then
      __TZ_SAFE="$(_sed_val "$__TZ")"
      _sedi "s|^  # timezone:.*$|  timezone: ${__TZ_SAFE}|" "$__YAML"
      _sedi "s|^  timezone: .*$|  timezone: ${__TZ_SAFE}|" "$__YAML"
      ok "harness.yaml: sandbox.timezone → $__TZ"
      unset __TZ_SAFE
    fi

    # git.user_name: uncomment if commented, then set value (idempotent)
    if [ -n "$__GIT_NAME" ]; then
      __GN_SAFE="$(_sed_val "$__GIT_NAME")"
      _sedi "s|^  # user_name:.*$|  user_name: ${__GN_SAFE}|" "$__YAML"
      _sedi "s|^  user_name: .*$|  user_name: ${__GN_SAFE}|" "$__YAML"
      ok "harness.yaml: git.user_name → $__GIT_NAME"
      unset __GN_SAFE
    fi

    # git.user_email: uncomment if commented, then set value (idempotent)
    if [ -n "$__GIT_EMAIL" ]; then
      __GE_SAFE="$(_sed_val "$__GIT_EMAIL")"
      _sedi "s|^  # user_email:.*$|  user_email: ${__GE_SAFE}|" "$__YAML"
      _sedi "s|^  user_email: .*$|  user_email: ${__GE_SAFE}|" "$__YAML"
      ok "harness.yaml: git.user_email → $__GIT_EMAIL"
      unset __GE_SAFE
    fi
...
printf "  ──────────────────────────────────────\n"
printf "       ${CYAN}harness.yaml${NC}        — sandbox name, timezone, git identity, install flags.\n"
printf "       ${CYAN}.devcontainer/.env${NC}  — secrets only (GH_TOKEN, Slack tokens).\n"
