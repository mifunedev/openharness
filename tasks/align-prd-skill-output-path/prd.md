...
The `/prd` skill currently names two different PRD output locations. Its early job step says to write `tasks/prd-[feature-name].md`, while the output/checklist section and downstream tooling expect `tasks/<feature-name>/prd.md`. Align the skill and add a deterministic guard so the pipeline keeps a single task-folder artifact contract.

## Goals

- Make `tasks/<feature-name>/prd.md` the only documented `/prd` output path.
- Remove the stale flat `tasks/prd-[feature-name].md` instruction.
...
- [x] `.claude/skills/prd/SKILL.md` no longer references `tasks/prd-[feature-name].md`.
- [x] The job step says to save to `tasks/<feature-name>/prd.md`.
- [x] Existing output/checklist language remains aligned with `tasks/<feature-name>/prd.md`.
...
- [x] The probe fails on `tasks/prd-[feature-name].md` in provider-facing PRD skill docs.
- [x] The probe passes only when `tasks/<feature-name>/prd.md` is present.
...
- [x] `tasks/align-prd-skill-output-path/prd.md` and `prd.json` describe the work.
...
- FR-1: `/prd` instructions must use `tasks/<feature-name>/prd.md` consistently.
- FR-2: The stale flat `tasks/prd-[feature-name].md` path must be absent from PRD skill docs.
