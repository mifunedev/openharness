# Ralph iteration — first-boot-banner-command

You are one iteration of a Ralph loop implementing the `first-boot-banner-command` task. The full plan is in `tasks/first-boot-banner-command/prd.md` and the structured task list is in `tasks/first-boot-banner-command/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/first-boot-banner-command/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/first-boot-banner-command/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/first-boot-banner-command/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-first-boot-banner-command` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-first-boot-banner-command origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-first-boot-banner-command
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-first-boot-banner-command`.
...
- PRD: `tasks/first-boot-banner-command/prd.md`
- Structured stories: `tasks/first-boot-banner-command/prd.json`
- Branch: `<prefix>/<N>-first-boot-banner-command`
- Issue: #124
