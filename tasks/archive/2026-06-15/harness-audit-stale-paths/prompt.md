# Ralph iteration — harness-audit-stale-paths

You are one iteration of a Ralph loop implementing the `harness-audit-stale-paths` task. The full plan is in `tasks/harness-audit-stale-paths/prd.md` and the structured task list is in `tasks/harness-audit-stale-paths/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/harness-audit-stale-paths/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/harness-audit-stale-paths/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/harness-audit-stale-paths/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `feat/43-harness-audit-stale-paths` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b feat/43-harness-audit-stale-paths origin/development 2>/dev/null \
     || git checkout feat/43-harness-audit-stale-paths
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/43-harness-audit-stale-paths`.
...
- PRD: `tasks/harness-audit-stale-paths/prd.md`
- Structured stories: `tasks/harness-audit-stale-paths/prd.json`
- Branch: `feat/43-harness-audit-stale-paths`
- Issue: #43
