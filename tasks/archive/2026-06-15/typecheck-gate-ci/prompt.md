# Ralph iteration — typecheck-gate-ci

You are one iteration of a Ralph loop implementing the `typecheck-gate-ci` task. The full plan is in `tasks/typecheck-gate-ci/prd.md` and the structured task list is in `tasks/typecheck-gate-ci/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/typecheck-gate-ci/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/typecheck-gate-ci/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/typecheck-gate-ci/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `feat/51-typecheck-gate-ci` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b feat/51-typecheck-gate-ci origin/development 2>/dev/null \
     || git checkout feat/51-typecheck-gate-ci
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/51-typecheck-gate-ci`.
...
- PRD: `tasks/typecheck-gate-ci/prd.md`
- Structured stories: `tasks/typecheck-gate-ci/prd.json`
- Branch: `feat/51-typecheck-gate-ci`
- Issue: #51
