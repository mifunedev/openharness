# Ralph iteration — avoid-shell-sourcing-slack

You are one iteration of a Ralph loop implementing the `avoid-shell-sourcing-slack` task. The full plan is in `tasks/avoid-shell-sourcing-slack/prd.md` and the structured task list is in `tasks/avoid-shell-sourcing-slack/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/avoid-shell-sourcing-slack/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/avoid-shell-sourcing-slack/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/avoid-shell-sourcing-slack/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/avoid-shell-sourcing-slack/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-avoid-shell-sourcing-slack` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-avoid-shell-sourcing-slack`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-avoid-shell-sourcing-slack origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-avoid-shell-sourcing-slack
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-avoid-shell-sourcing-slack`.
...
- PRD: `tasks/avoid-shell-sourcing-slack/prd.md`
- Structured stories: `tasks/avoid-shell-sourcing-slack/prd.json`
- Branch: `<prefix>/<N>-avoid-shell-sourcing-slack`
- Issue: #231
