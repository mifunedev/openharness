# Ralph iteration — close-no-pr-sessions

You are one iteration of a Ralph loop implementing the `close-no-pr-sessions` task. The full plan is in `tasks/close-no-pr-sessions/prd.md` and the structured task list is in `tasks/close-no-pr-sessions/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/close-no-pr-sessions/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/close-no-pr-sessions/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/close-no-pr-sessions/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-close-no-pr-sessions` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-close-no-pr-sessions`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-close-no-pr-sessions origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-close-no-pr-sessions
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-close-no-pr-sessions`.
...
- PRD: `tasks/close-no-pr-sessions/prd.md`
- Structured stories: `tasks/close-no-pr-sessions/prd.json`
- Branch: `<prefix>/<N>-close-no-pr-sessions`
- Issue: #209
