# Ralph iteration — detect-codex-zero-credit

You are one iteration of a Ralph loop implementing the `detect-codex-zero-credit` task. The full plan is in `tasks/detect-codex-zero-credit/prd.md` and the structured task list is in `tasks/detect-codex-zero-credit/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/detect-codex-zero-credit/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/detect-codex-zero-credit/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/detect-codex-zero-credit/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/detect-codex-zero-credit/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-detect-codex-zero-credit` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-detect-codex-zero-credit`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-detect-codex-zero-credit origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-detect-codex-zero-credit
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-detect-codex-zero-credit`.
...
- PRD: `tasks/detect-codex-zero-credit/prd.md`
- Structured stories: `tasks/detect-codex-zero-credit/prd.json`
- Branch: `<prefix>/<N>-detect-codex-zero-credit`
- Issue: #240
