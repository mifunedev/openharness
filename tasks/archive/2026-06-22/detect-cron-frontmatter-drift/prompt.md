# Ralph iteration — detect-cron-frontmatter-drift

You are one iteration of a Ralph loop implementing the `detect-cron-frontmatter-drift` task. The full plan is in `tasks/detect-cron-frontmatter-drift/prd.md` and the structured task list is in `tasks/detect-cron-frontmatter-drift/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/detect-cron-frontmatter-drift/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/detect-cron-frontmatter-drift/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/detect-cron-frontmatter-drift/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/detect-cron-frontmatter-drift/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-detect-cron-frontmatter-drift` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-detect-cron-frontmatter-drift`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-detect-cron-frontmatter-drift origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-detect-cron-frontmatter-drift
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-detect-cron-frontmatter-drift`.
...
- PRD: `tasks/detect-cron-frontmatter-drift/prd.md`
- Structured stories: `tasks/detect-cron-frontmatter-drift/prd.json`
- Branch: `<prefix>/<N>-detect-cron-frontmatter-drift`
- Issue: #225
