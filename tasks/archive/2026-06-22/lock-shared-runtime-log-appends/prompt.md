# Ralph iteration — lock-shared-runtime-log-appends

You are one iteration of a Ralph loop implementing the `lock-shared-runtime-log-appends` task. The full plan is in `tasks/lock-shared-runtime-log-appends/prd.md` and the structured task list is in `tasks/lock-shared-runtime-log-appends/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/lock-shared-runtime-log-appends/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/lock-shared-runtime-log-appends/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/lock-shared-runtime-log-appends/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/204-lock-shared-runtime-log-appends` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/204-lock-shared-runtime-log-appends`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/204-lock-shared-runtime-log-appends origin/development 2>/dev/null \
     || git checkout <prefix>/204-lock-shared-runtime-log-appends
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/204-lock-shared-runtime-log-appends`.
...
- PRD: `tasks/lock-shared-runtime-log-appends/prd.md`
- Structured stories: `tasks/lock-shared-runtime-log-appends/prd.json`
- Branch: `<prefix>/204-lock-shared-runtime-log-appends`
- Issue: #204
