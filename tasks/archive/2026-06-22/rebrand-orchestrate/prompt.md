# Ralph iteration — rebrand-orchestrate

You are one iteration of a Ralph loop implementing the `rebrand-orchestrate` task. The full plan is in `tasks/rebrand-orchestrate/prd.md` and the structured task list is in `tasks/rebrand-orchestrate/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/rebrand-orchestrate/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/rebrand-orchestrate/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/rebrand-orchestrate/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-rebrand-orchestrate` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-rebrand-orchestrate`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-rebrand-orchestrate origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-rebrand-orchestrate
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-rebrand-orchestrate`.
...
- PRD: `tasks/rebrand-orchestrate/prd.md`
- Structured stories: `tasks/rebrand-orchestrate/prd.json`
- Branch: `<prefix>/<N>-rebrand-orchestrate`
- Issue: #214
