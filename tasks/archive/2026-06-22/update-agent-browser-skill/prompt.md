# Ralph iteration — update-agent-browser-skill

You are one iteration of a Ralph loop implementing the `update-agent-browser-skill` task. The full plan is in `tasks/update-agent-browser-skill/prd.md` and the structured task list is in `tasks/update-agent-browser-skill/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/update-agent-browser-skill/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/update-agent-browser-skill/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/update-agent-browser-skill/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-update-agent-browser-skill` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-update-agent-browser-skill`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-update-agent-browser-skill origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-update-agent-browser-skill
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-update-agent-browser-skill`.
...
- PRD: `tasks/update-agent-browser-skill/prd.md`
- Structured stories: `tasks/update-agent-browser-skill/prd.json`
- Branch: `<prefix>/<N>-update-agent-browser-skill`
- Issue: #148
