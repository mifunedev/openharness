# Ralph iteration — ci-harness-pr-coverage

You are one iteration of a Ralph loop implementing the `ci-harness-pr-coverage` task. The full plan is in `tasks/ci-harness-pr-coverage/prd.md` and the structured task list is in `tasks/ci-harness-pr-coverage/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/ci-harness-pr-coverage/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/ci-harness-pr-coverage/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/ci-harness-pr-coverage/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `feat/12-ci-harness-pr-coverage` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b feat/12-ci-harness-pr-coverage origin/development 2>/dev/null \
     || git checkout feat/12-ci-harness-pr-coverage
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/12-ci-harness-pr-coverage`.
...
- PRD: `tasks/ci-harness-pr-coverage/prd.md`
- Structured stories: `tasks/ci-harness-pr-coverage/prd.json`
- Branch: `feat/12-ci-harness-pr-coverage`
- Issue: #12
