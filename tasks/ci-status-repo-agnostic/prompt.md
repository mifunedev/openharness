# Ralph iteration — ci-status-repo-agnostic

You are one iteration of a Ralph loop implementing the `ci-status-repo-agnostic` task. The full plan is in `tasks/ci-status-repo-agnostic/prd.md` and the structured task list is in `tasks/ci-status-repo-agnostic/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/ci-status-repo-agnostic/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/ci-status-repo-agnostic/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/ci-status-repo-agnostic/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `feat/20-ci-status-repo-agnostic` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b feat/20-ci-status-repo-agnostic origin/development 2>/dev/null \
     || git checkout feat/20-ci-status-repo-agnostic
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/20-ci-status-repo-agnostic`.
...
- PRD: `tasks/ci-status-repo-agnostic/prd.md`
- Structured stories: `tasks/ci-status-repo-agnostic/prd.json`
- Branch: `feat/20-ci-status-repo-agnostic`
- Issue: #20
