# Ralph iteration — cleanup-tasks-scoped-guard

You are one iteration of a Ralph loop implementing the `cleanup-tasks-scoped-guard` task. The full plan is in `tasks/cleanup-tasks-scoped-guard/prd.md` and the structured task list is in `tasks/cleanup-tasks-scoped-guard/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/cleanup-tasks-scoped-guard/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/cleanup-tasks-scoped-guard/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/cleanup-tasks-scoped-guard/critique.md` — if present, the critic findings the stories must satisfy.
   - `.claude/rules/git.md` for branch + commit conventions, especially § "Isolating in-flight work" and § "Worktrees".
   ...
   - The file under change: `crons/cleanup-tasks.md`. The probe template to mirror: `evals/probes/owned-surface-guard.sh`.

2. **Verify branch** — your branch is `feat/85-cleanup-tasks-scoped-guard` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b feat/85-cleanup-tasks-scoped-guard origin/development 2>/dev/null \
     || git checkout feat/85-cleanup-tasks-scoped-guard
   ```
   Never push to `development` or `main` directly.

3. **Implement the chosen story** — make the file changes, additions, deletions specified in the story's `acceptanceCriteria`. Confine the work to that story; resist scope creep. This is a harness-infra task — you are editing `crons/cleanup-tasks.md` (markdown), adding a shell probe under `evals/probes/`, and touching `.worktrees/README.md` + `CHANGELOG.md`. Do NOT touch sandbox application code.
...
4. **Run quality checks** before commit:
   ...
   ```
   For the probe story (US-003), additionally verify the probe runs green:
   ```bash
   chmod +x evals/probes/cleanup-tasks-scoped-guard.sh
   bash evals/probes/cleanup-tasks-scoped-guard.sh; echo "exit: $?"   # expect PASS / exit 0
   ```
...
5. **Commit** with this message format (per `.claude/rules/git.md`):
   ...
   ```
   Where `<type>` is `task` for most stories, `feat` for net-new functionality, `fix` for bugs. Example: `fix: US-001 — Scope the cleanup-tasks pre-flight to tasks/`. Stage only files touched by this story.
...
- **Never push** to `development` or `main`. Branch is `feat/85-cleanup-tasks-scoped-guard`.
...
- **Harness-infra scope only.** This task touches `crons/`, `evals/`, `.worktrees/README.md`, and `CHANGELOG.md`. Never write sandbox application code.
- **Phase ordering matters.** Stories are priority-ordered by dependency — implement them in `priority` order. US-003 (probe) validates the cron edits from US-001/US-002, so it must come after them.
- **CHANGELOG discipline.** Per `.claude/rules/git.md`, the CHANGELOG entry is its own story (US-004) under `### Fixed`.
- **Don't modify completed stories** unless the current story explicitly requires it.

## Reference

- PRD: `tasks/cleanup-tasks-scoped-guard/prd.md`
- Structured stories: `tasks/cleanup-tasks-scoped-guard/prd.json`
- Branch: `feat/85-cleanup-tasks-scoped-guard`
- Issue: #85
