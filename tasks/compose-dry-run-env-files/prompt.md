...
You are one iteration of a Ralph loop implementing the `compose-dry-run-env-files` task. The full plan is in `tasks/compose-dry-run-env-files/prd.md` and the structured task list is in `tasks/compose-dry-run-env-files/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/compose-dry-run-env-files/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/compose-dry-run-env-files/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/compose-dry-run-env-files/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/compose-dry-run-env-files/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
- PRD: `tasks/compose-dry-run-env-files/prd.md`
- Structured stories: `tasks/compose-dry-run-env-files/prd.json`
