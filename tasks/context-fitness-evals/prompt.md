# Ralph iteration — context-fitness-evals

You are one iteration of a Ralph loop implementing the `context-fitness-evals` task. The full plan is in `tasks/context-fitness-evals/prd.md` and the structured task list is in `tasks/context-fitness-evals/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/context-fitness-evals/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/context-fitness-evals/prd.md` — read **§10 Critique Mitigations**; the `[M-N]` tags in the story acceptanceCriteria are binding and override conflicting criteria above.
   - `tasks/context-fitness-evals/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/context-fitness-evals/critique.md` — the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `feat/1-context-fitness-evals` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b feat/1-context-fitness-evals origin/development 2>/dev/null \
     || git checkout feat/1-context-fitness-evals
   ```
   Never push to `development` or `main` directly.

3. **Implement the chosen story** — make the file changes, additions, deletions specified in the story's `acceptanceCriteria`. Confine the work to that story; resist scope creep. **Orchestrator boundary:** every artifact lives under `evals/`, `.claude/skills/`, `.claude/hooks/`, `scripts/`, or `crons/` — harness infrastructure, not sandbox application code.

4. **Run quality checks** before commit:
   ```bash
   shellcheck evals/probes/*.sh scripts/*.sh .claude/hooks/*.sh 2>/dev/null || true
...
5. **Commit** with this message format (per `.claude/rules/git.md`):
   ...
   ```
   Where `<type>` is `feat` for net-new functionality, `task` for scaffolding, `fix` for bugs. Example: `feat: US-001 — establish evals/ corpus and probe contract`. Stage only files touched by this story.
...
- **Never push** to `development` or `main`. Branch is `feat/1-context-fitness-evals`. `origin` is the private fork `ryaneggz/openharness`.
...
- **EXTEND, never delete** the existing behavior of `/context-audit`, `/health-check`, `/retro`, `crons/heartbeat.md`, `crons/cleanup-tasks.md` (FR-10). US-006 makes `/context-audit` *source* a shared helper; it must not change Tier-2's behavior.
- **CHANGELOG discipline.** Per `.claude/rules/git.md`, every story with user-visible impact must add an entry under `## [Unreleased]` in the same commit.
- **Don't modify completed stories** unless the current story explicitly requires it.

## Reference

- PRD: `tasks/context-fitness-evals/prd.md` (note binding §10 mitigations)
- Structured stories: `tasks/context-fitness-evals/prd.json`
- Branch: `feat/1-context-fitness-evals`
- Issue: #1
