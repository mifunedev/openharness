# Ralph iteration — correct-harness-audit-memory-path

You are one iteration of a Ralph loop implementing the `correct-harness-audit-memory-path` task. The full plan is in `tasks/correct-harness-audit-memory-path/prd.md` and the structured task list is in `tasks/correct-harness-audit-memory-path/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/correct-harness-audit-memory-path/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/correct-harness-audit-memory-path/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/correct-harness-audit-memory-path/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-correct-harness-audit-memory-path` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-correct-harness-audit-memory-path`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-correct-harness-audit-memory-path origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-correct-harness-audit-memory-path
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-correct-harness-audit-memory-path`.
...
- PRD: `tasks/correct-harness-audit-memory-path/prd.md`
- Structured stories: `tasks/correct-harness-audit-memory-path/prd.json`
- Branch: `<prefix>/<N>-correct-harness-audit-memory-path`
- Issue: #176
