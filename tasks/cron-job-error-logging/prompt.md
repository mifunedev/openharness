# Ralph iteration — cron-job-error-logging

You are one iteration of a Ralph loop implementing the `cron-job-error-logging` task. The full plan is in `tasks/cron-job-error-logging/prd.md` and the structured task list is in `tasks/cron-job-error-logging/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/cron-job-error-logging/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/cron-job-error-logging/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/cron-job-error-logging/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-cron-job-error-logging` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-cron-job-error-logging origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-cron-job-error-logging
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-cron-job-error-logging`.
...
- PRD: `tasks/cron-job-error-logging/prd.md`
- Structured stories: `tasks/cron-job-error-logging/prd.json`
- Branch: `<prefix>/<N>-cron-job-error-logging`
- Issue: #49
