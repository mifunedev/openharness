# Ralph iteration — cron-sighup-reschedule

You are one iteration of a Ralph loop implementing the `cron-sighup-reschedule` task. The full plan is in `tasks/cron-sighup-reschedule/prd.md` and the structured task list is in `tasks/cron-sighup-reschedule/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/cron-sighup-reschedule/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/cron-sighup-reschedule/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/cron-sighup-reschedule/critique.md` — if present, the critic findings the stories must satisfy. THIS TASK WAS CRITIC-GATED — honor those findings.
   - `.claude/rules/git.md` for branch + commit conventions.
   - `.claude/rules/advisor-model.md` for any critic-gated story.
   - The implementation target is `scripts/cron-runtime.ts` and its test `scripts/__tests__/cron-runtime.test.ts`; the doc target is `crons/README.md`. This is harness-infra (orchestrator scripts/crons/docs) — NOT sandbox application code.

2. **Verify branch** — your branch is `feat/88-cron-sighup-reschedule` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b feat/88-cron-sighup-reschedule origin/development 2>/dev/null \
     || git checkout feat/88-cron-sighup-reschedule
   ```
...
4. **Run quality checks** before commit (use the real root script names from package.json):
   ```bash
   pnpm run typecheck 2>/dev/null || true
   pnpm run lint 2>/dev/null || true
   pnpm run test 2>/dev/null || true
   ```
   The cron-runtime tests are exercised by `pnpm run test` (vitest, includes `scripts/__tests__/cron-runtime.test.ts`). If checks fail, fix them. Do not commit broken code.
...
5. **Commit** with this message format (per `.claude/rules/git.md`):
   ...
   ```
   Where `<type>` is `feat` for net-new functionality (US-001..US-004), `task`/`feat` for docs (US-005). Example: `feat: US-001 — Track live job handles ...`. Stage only files touched by this story.
...
- **Never push** to `development` or `main`. Branch is `feat/88-cron-sighup-reschedule`.
...
- **Phase ordering matters.** Stories are priority-ordered by dependency (US-001 registry → US-002 handler → US-003 logging → US-004 tests → US-005 docs) — implement them in `priority` order.
- **CHANGELOG discipline.** Per `.claude/rules/git.md`, US-005 adds the `## [Unreleased]` → `### Added` entry linking #88.
- **Don't modify completed stories** unless the current story explicitly requires it.

## Reference

- PRD: `tasks/cron-sighup-reschedule/prd.md`
- Structured stories: `tasks/cron-sighup-reschedule/prd.json`
- Critique: `tasks/cron-sighup-reschedule/critique.md`
- Branch: `feat/88-cron-sighup-reschedule`
- Issue: #88
