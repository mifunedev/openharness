# Ralph iteration — debugmcp-integration

You are one iteration of a Ralph loop implementing the `debugmcp-integration` task. The full plan is in `tasks/debugmcp-integration/prd.md` and the structured task list is in `tasks/debugmcp-integration/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/debugmcp-integration/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/debugmcp-integration/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/debugmcp-integration/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/debugmcp-integration/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `feat/297-debugmcp-integration` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/feat/297-debugmcp-integration`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b feat/297-debugmcp-integration "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout feat/297-debugmcp-integration
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/297-debugmcp-integration`.
...
- PRD: `tasks/debugmcp-integration/prd.md`
- Structured stories: `tasks/debugmcp-integration/prd.json`
- Branch: `feat/297-debugmcp-integration`
- Issue: #297
