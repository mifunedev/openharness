# Ralph iteration — dedupe-merged-autopilot-refs

You are one iteration of a Ralph loop implementing the `dedupe-merged-autopilot-refs` task. The full plan is in `tasks/dedupe-merged-autopilot-refs/prd.md` and the structured task list is in `tasks/dedupe-merged-autopilot-refs/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/dedupe-merged-autopilot-refs/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/dedupe-merged-autopilot-refs/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/dedupe-merged-autopilot-refs/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/dedupe-merged-autopilot-refs/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-dedupe-merged-autopilot-refs` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-dedupe-merged-autopilot-refs`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
...
- PRD: `tasks/dedupe-merged-autopilot-refs/prd.md`
- Structured stories: `tasks/dedupe-merged-autopilot-refs/prd.json`
