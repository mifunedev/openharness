# Ralph iteration — devcontainer-ci-gate

You are one iteration of a Ralph loop implementing the `devcontainer-ci-gate` task. The full plan is in `tasks/devcontainer-ci-gate/prd.md` and the structured task list is in `tasks/devcontainer-ci-gate/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/devcontainer-ci-gate/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/devcontainer-ci-gate/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/devcontainer-ci-gate/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `feat/26-devcontainer-ci-gate` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b feat/26-devcontainer-ci-gate origin/development 2>/dev/null \
     || git checkout feat/26-devcontainer-ci-gate
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/26-devcontainer-ci-gate`.
...
- PRD: `tasks/devcontainer-ci-gate/prd.md`
- Structured stories: `tasks/devcontainer-ci-gate/prd.json`
- Branch: `feat/26-devcontainer-ci-gate`
- Issue: #26
