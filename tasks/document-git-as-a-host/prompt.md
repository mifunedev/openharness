# Ralph iteration — document-git-as-a-host

You are one iteration of a Ralph loop implementing the `document-git-as-a-host` task. The full plan is in `tasks/document-git-as-a-host/prd.md` and the structured task list is in `tasks/document-git-as-a-host/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/document-git-as-a-host/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/document-git-as-a-host/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/document-git-as-a-host/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-document-git-as-a-host` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-document-git-as-a-host`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-document-git-as-a-host origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-document-git-as-a-host
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-document-git-as-a-host`.
...
- PRD: `tasks/document-git-as-a-host/prd.md`
- Structured stories: `tasks/document-git-as-a-host/prd.json`
- Branch: `<prefix>/<N>-document-git-as-a-host`
- Issue: #217
