# Ralph iteration — expand-harness-ci-path-filters

You are one iteration of a Ralph loop implementing the `expand-harness-ci-path-filters` task. The full plan is in `tasks/expand-harness-ci-path-filters/prd.md` and the structured task list is in `tasks/expand-harness-ci-path-filters/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/expand-harness-ci-path-filters/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/expand-harness-ci-path-filters/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/expand-harness-ci-path-filters/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `feat/165-expand-harness-ci-path-filters` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/feat/165-expand-harness-ci-path-filters`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b feat/165-expand-harness-ci-path-filters origin/development 2>/dev/null \
     || git checkout feat/165-expand-harness-ci-path-filters
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/165-expand-harness-ci-path-filters`.
...
- PRD: `tasks/expand-harness-ci-path-filters/prd.md`
- Structured stories: `tasks/expand-harness-ci-path-filters/prd.json`
- Branch: `feat/165-expand-harness-ci-path-filters`
- Issue: #165
