# Ralph iteration — fail-harness-audit-on-empty

You are one iteration of a Ralph loop implementing the `fail-harness-audit-on-empty` task. The full plan is in `tasks/fail-harness-audit-on-empty/prd.md` and the structured task list is in `tasks/fail-harness-audit-on-empty/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/fail-harness-audit-on-empty/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/fail-harness-audit-on-empty/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/fail-harness-audit-on-empty/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/fail-harness-audit-on-empty/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-fail-harness-audit-on-empty` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-fail-harness-audit-on-empty`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-fail-harness-audit-on-empty origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-fail-harness-audit-on-empty
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-fail-harness-audit-on-empty`.
...
- PRD: `tasks/fail-harness-audit-on-empty/prd.md`
- Structured stories: `tasks/fail-harness-audit-on-empty/prd.json`
- Branch: `<prefix>/<N>-fail-harness-audit-on-empty`
- Issue: #246
