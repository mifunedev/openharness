# Ralph iteration — installer-local-host-defaults

You are one iteration of a Ralph loop implementing the `installer-local-host-defaults` task. The full plan is in `tasks/installer-local-host-defaults/prd.md` and the structured task list is in `tasks/installer-local-host-defaults/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/installer-local-host-defaults/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/installer-local-host-defaults/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/installer-local-host-defaults/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-installer-local-host-defaults` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-installer-local-host-defaults`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-installer-local-host-defaults origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-installer-local-host-defaults
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-installer-local-host-defaults`.
...
- PRD: `tasks/installer-local-host-defaults/prd.md`
- Structured stories: `tasks/installer-local-host-defaults/prd.json`
- Branch: `<prefix>/<N>-installer-local-host-defaults`
- Issue: #211
