# Ralph iteration — make-cron-singleton-lock

You are one iteration of a Ralph loop implementing the `make-cron-singleton-lock` task. The full plan is in `tasks/make-cron-singleton-lock/prd.md` and the structured task list is in `tasks/make-cron-singleton-lock/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/make-cron-singleton-lock/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/make-cron-singleton-lock/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/make-cron-singleton-lock/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/make-cron-singleton-lock/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-make-cron-singleton-lock` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-make-cron-singleton-lock`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-make-cron-singleton-lock origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-make-cron-singleton-lock
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-make-cron-singleton-lock`.
...
- PRD: `tasks/make-cron-singleton-lock/prd.md`
- Structured stories: `tasks/make-cron-singleton-lock/prd.json`
- Branch: `<prefix>/<N>-make-cron-singleton-lock`
- Issue: #233
