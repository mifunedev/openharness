# Ralph iteration — make-harness-audit-worktree-aware

You are one iteration of a Ralph loop implementing the `make-harness-audit-worktree-aware` task. The full plan is in `tasks/make-harness-audit-worktree-aware/prd.md` and the structured task list is in `tasks/make-harness-audit-worktree-aware/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/make-harness-audit-worktree-aware/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/make-harness-audit-worktree-aware/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/make-harness-audit-worktree-aware/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-make-harness-audit-worktree-aware` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-make-harness-audit-worktree-aware`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-make-harness-audit-worktree-aware origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-make-harness-audit-worktree-aware
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-make-harness-audit-worktree-aware`.
...
- PRD: `tasks/make-harness-audit-worktree-aware/prd.md`
- Structured stories: `tasks/make-harness-audit-worktree-aware/prd.json`
- Branch: `<prefix>/<N>-make-harness-audit-worktree-aware`
- Issue: #183
