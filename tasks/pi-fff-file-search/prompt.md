# Ralph iteration — pi-fff-file-search

You are one iteration of a Ralph loop implementing the `pi-fff-file-search` task. The full plan is in `tasks/pi-fff-file-search/prd.md` and the structured task list is in `tasks/pi-fff-file-search/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/pi-fff-file-search/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/pi-fff-file-search/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/pi-fff-file-search/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/pi-fff-file-search/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `feat/261-pi-fff-file-search` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/feat/261-pi-fff-file-search`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ...
   git checkout -b feat/261-pi-fff-file-search "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout feat/261-pi-fff-file-search
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/261-pi-fff-file-search`.
...
- PRD: `tasks/pi-fff-file-search/prd.md`
- Structured stories: `tasks/pi-fff-file-search/prd.json`
- Branch: `feat/261-pi-fff-file-search`
- Issue: #261
