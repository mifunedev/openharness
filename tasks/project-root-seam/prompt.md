# Ralph iteration — project-root-seam

You are one iteration of a Ralph loop implementing the `project-root-seam` task. The full plan is in `tasks/project-root-seam/prd.md` and the structured task list is in `tasks/project-root-seam/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/project-root-seam/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/project-root-seam/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/project-root-seam/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/project-root-seam/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-project-root-seam` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-project-root-seam`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b <prefix>/<N>-project-root-seam "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout <prefix>/<N>-project-root-seam
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-project-root-seam`.
...
- PRD: `tasks/project-root-seam/prd.md`
- Structured stories: `tasks/project-root-seam/prd.json`
- Branch: `<prefix>/<N>-project-root-seam`
- Issue: #531
