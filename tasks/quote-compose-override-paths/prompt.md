# Ralph iteration — quote-compose-override-paths

You are one iteration of a Ralph loop implementing the `quote-compose-override-paths` task. The full plan is in `tasks/quote-compose-override-paths/prd.md` and the structured task list is in `tasks/quote-compose-override-paths/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/quote-compose-override-paths/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/quote-compose-override-paths/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/quote-compose-override-paths/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `<prefix>/<N>-quote-compose-override-paths` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-quote-compose-override-paths`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-quote-compose-override-paths origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-quote-compose-override-paths
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-quote-compose-override-paths`.
...
- PRD: `tasks/quote-compose-override-paths/prd.md`
- Structured stories: `tasks/quote-compose-override-paths/prd.json`
- Branch: `<prefix>/<N>-quote-compose-override-paths`
- Issue: #185
