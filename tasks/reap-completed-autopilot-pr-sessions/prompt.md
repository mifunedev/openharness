# Ralph iteration — reap-completed-autopilot-pr-sessions

You are one iteration of a Ralph loop implementing the `reap-completed-autopilot-pr-sessions` task. The full plan is in `tasks/reap-completed-autopilot-pr-sessions/prd.md` and the structured task list is in `tasks/reap-completed-autopilot-pr-sessions/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/reap-completed-autopilot-pr-sessions/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/reap-completed-autopilot-pr-sessions/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/reap-completed-autopilot-pr-sessions/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/reap-completed-autopilot-pr-sessions/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `<prefix>/<N>-reap-completed-autopilot-pr-sessions` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.worktrees/<prefix>/<N>-reap-completed-autopilot-pr-sessions`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   git fetch origin
   git checkout -b <prefix>/<N>-reap-completed-autopilot-pr-sessions origin/development 2>/dev/null \
     || git checkout <prefix>/<N>-reap-completed-autopilot-pr-sessions
   ```
...
- **Never push** to `development` or `main`. Branch is `<prefix>/<N>-reap-completed-autopilot-pr-sessions`.
...
- PRD: `tasks/reap-completed-autopilot-pr-sessions/prd.md`
- Structured stories: `tasks/reap-completed-autopilot-pr-sessions/prd.json`
- Branch: `<prefix>/<N>-reap-completed-autopilot-pr-sessions`
- Issue: #235
