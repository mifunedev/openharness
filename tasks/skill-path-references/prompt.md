# Ralph iteration — skill-path-references

You are one iteration of a Ralph loop implementing the `skill-path-references` task. The full plan is in `tasks/skill-path-references/prd.md` and the structured task list is in `tasks/skill-path-references/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/skill-path-references/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/skill-path-references/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/skill-path-references/critique.md` — if present, the critic findings the stories must satisfy.
...
2. **Verify branch** — your branch is `feat/69-skill-path-references` (per `prd.json` `branchName`). If you are not on it:
   ```bash
   git fetch origin
   git checkout -b feat/69-skill-path-references origin/development 2>/dev/null \
     || git checkout feat/69-skill-path-references
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/69-skill-path-references`.
...
- PRD: `tasks/skill-path-references/prd.md`
- Structured stories: `tasks/skill-path-references/prd.json`
- Branch: `feat/69-skill-path-references`
- Issue: #69
