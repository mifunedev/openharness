...
You are one iteration of a Ralph loop implementing the `smoke-test-release-image` task. The full plan is in `tasks/smoke-test-release-image/prd.md` and the structured task list is in `tasks/smoke-test-release-image/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `tasks/smoke-test-release-image/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `tasks/smoke-test-release-image/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `tasks/smoke-test-release-image/critique.md` — if present, the critic findings the stories must satisfy.
   - If `tasks/smoke-test-release-image/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
- PRD: `tasks/smoke-test-release-image/prd.md`
- Structured stories: `tasks/smoke-test-release-image/prd.json`
