...
- `scripts/cron-runtime.ts:235-277` — `reloadEntryForFire` re-reads fire-time execution metadata and must preserve the canonical absolute `filePath` so later body reloads keep reading the cron file, not a basename in the process cwd.
...
## Detail
A cron file has two lifecycles. Its leading frontmatter becomes scheduler/config state: `parseCronFile` maps `schedule`, `enabled`, `tmux`, `worktree`, `agent`, `preflight`, and `repo` into a `CronEntry` while resolving the source path to an absolute `filePath` (`scripts/cron-runtime.ts:76-101`). `loadCrons` then drops entries that the runtime will not arm: disabled files, invalid ids, filename/id mismatches, unsafe agent/repo/remote overrides, and invalid schedules (`scripts/cron-runtime.ts:127-176`). Those filters define what counts as a schedulable cron.
