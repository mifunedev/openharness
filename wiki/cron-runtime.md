...
- `scripts/cron-runtime.ts:76-101` — frontmatter parsing maps `schedule`, `enabled`, `tmux`, `worktree`, `agent`, `preflight`, and `repo` into `CronEntry` while preserving the caller-provided source file path.
...
## Summary
Open Harness' cron runtime is a small scheduler that reads `crons/*.md` frontmatter into `CronEntry` records, uses the markdown body as the agent prompt, and protects the scheduler with a singleton pidfile lock. Body text hot-reloads at fire time through the entry's path-qualified `filePath`, and selected execution metadata (`preflight`, `repo`, agent, tmux/worktree flags) is re-read immediately before a fire; the already-armed schedule cadence still requires a SIGHUP reschedule or runtime restart to change. Runtime and shell-wrapper liveness records are serialized through `scripts/locked-append.sh` so concurrent fires write whole `.cron.log` records.
